...
const Projects = () => {
  ...
  const projects: Project[] = [{
    ...
    description: 'A sophisticated content generation platform using fine-tuned GPT models to create industry-specific marketing materials.',
    image: 'https://images.unsplash.com/photo-1620712943543-bcc4688e7485?auto=format&fit=crop&q=80&w=800',
    tags: ['LLM', 'NLP', 'Web App'],
    liveLink: '#',
    githubLink: '#'
  }, {
    id: 2,
    title: 'Intelligent Document Analyzer',
    description: 'An automated system for extracting, categorizing, and analyzing information from complex business documents.',
    image: 'https://images.unsplash.com/photo-1581291518633-83b4ebd1d83e?auto=format&fit=crop&q=80&w=800',
    tags: ['Computer Vision', 'NLP', 'API'],
    liveLink: '#',
    githubLink: '#'
  }, {
    id: 3,
    title: 'Personalized Learning Platform',
    description: 'An adaptive educational platform that tailors content to individual learning styles using AI algorithms.',
    image: 'https://images.unsplash.com/photo-1501504905252-473c47e087f8?auto=format&fit=crop&q=80&w=800',
    tags: ['ML', 'EdTech', 'Web App'],
    liveLink: '#',
    githubLink: '#'
  }, {
    id: 4,
    title: 'Predictive Customer Analytics',
    description: 'A system that forecasts customer behavior and preferences using advanced machine learning techniques.',
    image: 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&q=80&w=800',
    tags: ['ML', 'Analytics', 'API'],
    liveLink: '#',
    githubLink: '#'
  }];
  const filters = ['All', 'LLM', 'NLP', 'ML', 'Web App', 'API'];
  ...
  return <section id="projects" className="py-20 bg-gray-950 relative">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="max-w-xl mx-auto text-center mb-16">
          ...
          <p className="text-gray-300">
            Explore a selection of innovative solutions I've developed for
            clients across various industries.
          </p>
        </div>
        {/* Project Filters */}
        <div className="flex flex-wrap justify-center gap-2 mb-12">
          {filters.map(filter => <button key={filter} onClick={() => setActiveFilter(filter)} className={`px-4 py-2 rounded-full text-sm transition-all duration-300 ${activeFilter === filter ? 'bg-lime-400 text-black' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}>
              {filter}
            </button>)}
        </div>
        {/* Projects Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
          {filteredProjects.map((project, index) => <div key={project.id} className="group relative" onMouseEnter={() => setHoveredProject(project.id)} onMouseLeave={() => handleMouseLeave(index)} onMouseMove={e => handleMouseMove(e, index)} ref={el => projectRefs.current[index] = el} style={{
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden relative z-10">
                <div className="relative h-64 overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-end">
                    <div className="p-6 w-full flex justify-between items-center">
                      {project.liveLink && <a href={project.liveLink} target="_blank" rel="noopener noreferrer" className="text-white bg-lime-400/20 hover:bg-lime-400/30 p-2 rounded-full transition-all duration-300">
                          <ExternalLinkIcon size={20} />
                        </a>}
                      {project.githubLink && <a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="text-white bg-gray-800/80 hover:bg-gray-700 p-2 rounded-full transition-all duration-300">
                          <GithubIcon size={20} />
                        </a>}
                    </div>
                  </div>
                ...
                <div className="p-6">
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.tags.map(tag => <span key={tag} className="text-xs px-2 py-1 bg-gray-800 text-gray-300 rounded-full">
                        {tag}
                      </span>)}
                  </div>
